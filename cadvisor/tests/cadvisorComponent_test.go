...
func runDeisRouterTest(
	...
	dockercliutils.BuildDockerfile(t, "../", "deis/cadvisor:"+testSessionID)
	...
	go func() {
		...
		dockercliutils.RunContainer(t, cli,
			"--name", "deis-cadvisor-"+testSessionID,
			"-p", servicePort+":80",
			"-p", "2222:2222",
			"-e", "PUBLISH="+servicePort,
			"-e", "HOST="+IPAddress,
			"-e", "ETCD_PORT="+etcdPort,
			"deis/cadvisor:"+testSessionID)
	}()
	time.Sleep(2000 * time.Millisecond)
	dockercliutils.PrintToStdout(t, stdout, stdoutPipe, "deis-cadvisor running")
}
